...
def test_history_deque_limit_and_rotation(service):
    service.history.appendleft("test1")
    service.history.appendleft("test2")
    service.history.appendleft("test3")
    service.history.appendleft("test4")
    
    assert len(service.history) == 3 # Should not exceed history_limit=3
    assert service.history[0] == "test4" # Latest is test4
    assert "test1" not in service.history # First inserted string should be rotated out

def test_clear_memory(service):
    service.history.appendleft("secret_cookie_token")
    service._last_clip = "secret_cookie_token"
    
    service.clear_memory()
    
    assert len(service.history) == 0
    assert service._last_clip is None
